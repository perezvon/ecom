...
const camelCaseify = (string) => {
  string =
    string.slice(0, 3) !== 'SEO'
      ? string.charAt(0).toLowerCase() + string.slice(1)
      : string.slice(0, 3).toLowerCase() + string.slice(3);
...
db.once('open', async () => {
  ...
  data.forEach((product) => {
    ...
    product.forEach((field, i) => {
      ...
      if (numericalCategories.find((cat) => cat === header))
        productObj[header] = Number(field.replace(/"/gm, '').trim());
      else productObj[header] = field.replace(/"/gm, '').trim();
    });
    productObj.productID = productObj.productId;
    productObj.categories = [
      `${productObj.store}:${productObj.category}`,
      ...productObj.secondaryCategories.split(';').map((c) => c.trim()),
    ];
    productObj.totalInventory = productObj.quantity;
    productObj.availableStores = [
      (stores.find((s) => s.name === productObj.store) || {}).storeID || '',
    ];
    const existingProduct = products.find(
      (p) => p.productID === productObj.productID
    );
